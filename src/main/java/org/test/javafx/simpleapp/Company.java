package org.test.javafx.simpleapp;
...
public class Company extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		ageFilterBox.getItems().addAll("no matter", "over 20", "over 30", "over 40");

		salaryFilterBox.getItems().addAll("no matter", "over 150", "over 250", "over 500");

		relationshipFilterBox.getItems().addAll("no matter", "married", "single");
		...
		buttonGetInfo.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent e) {
				...
				if (u != null) {
					textInfo.setText("Age is " + u.getAge() + ", " + "Salary is " + u.getSalary() + ", "
		...
		buttonAddUser.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent e) {
				...
				User u = new User(name.getText(), Integer.parseInt(age.getText()), Integer.parseInt(salary.getText()),
						mm);
				users.add(u);
		...
		filter.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent e) {
				...
				// resultFilter.setText(String.valueOf(age) + " " +
				// String.valueOf(salary) + " "+ relate);

				List<User> list;
				if (index != 0) {
					list = users.stream().filter(u -> u.age > age).filter(u -> u.salary > salary).filter(u -> u.married == relate).collect(Collectors.toList());
				} else {
					list = users.stream().filter(u -> u.age > age).filter(u -> u.salary > salary).collect(Collectors.toList());
				}
